    ...
    <nav className="bg-[#1E4636] shadow-md fixed w-full z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16 items-center">
          {/* Logo */}
          <NavLink to="/" className="text-2xl font-bold text-[#13AA52]">MyLogo</NavLink>
      ...
      {/* Mobile Menu */}
      <div className={`${isOpen ? "block" : "hidden"} md:hidden bg-[#1E4636] p-4 space-y-3 transition-all duration-300`}>
        {["Men", "Women", "Bestseller"].map((item) => (
          <NavLink
            key={item}
            to={`/${item.toLowerCase()}`}
